...
public class RemittanceRepository : IRemittanceRepository
{
	...
	public void AddRemittance(AddUpdateRemittanceModel remittanceModel)
	{
		...
		_context.Remittances.Add(remittance);
		 ...
		 UpdateExpenseIncome(remittanceModel.SubmitDate, netProfit);

		 _context.SaveChanges();
	}

	public void UpdateRemittance(int remittanceId, AddUpdateRemittanceModel remittanceModel)
	{
		var remittance = GetRemittanceById(remittanceId);
		...
		{
			 UpdateExpenseIncome(remittanceModel.SubmitDate, netProfitDifference);
		}
		else
		{
			UpdateExpenseIncome(remittanceModel.SubmitDate, netProfitDifference);
	...
	public void DeleteRemittance(int remittanceId)
	{
		var remittance = GetRemittanceById(remittanceId);

		UpdateExpenseIncome(remittance.SubmitDate, -remittance.NetProfit);
	...
	private Remittance GetRemittanceById(int remittanceId)
	{
		var remittance =  _context.Remittances.FirstOrDefault(i => i.Id == remittanceId);
		if (remittance == null)
		{
	...
	private void UpdateExpenseIncome(DateTime submitDate, long netProfit)
	{
		...
		var expense = _context.Expenses.FirstOrDefault(e => e.SubmitDate == submitDate);
